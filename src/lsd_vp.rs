...
impl Engine {
    ...
    pub fn infer(&mut self, l: &ImageF32) -> Option<Hypothesis> {
        ...
        let mut fam1_idx: Vec<usize> = Vec::new();
        let mut fam2_idx: Vec<usize> = Vec::new();
        ...
        if fam1_idx.len() < 6 || fam2_idx.len() < 6 {
            debug!(
        ...
        let vpu = estimate_vp(&segs, &fam1_idx)?; // u-direction VP
        let vpv = estimate_vp(&segs, &fam2_idx)?; // v-direction VP
        ...
        let conf = ((fam1_idx.len().min(50) as f32 / 50.0)
            * (fam2_idx.len().min(50) as f32 / 50.0)
        ...
        let diagnostics = build_lsd_diagnostics(&segs, &families, a1, a2, conf, elapsed_ms);
        Some(Hypothesis {
            hmtx0,
            confidence: conf,
            diagnostics,
...
fn estimate_vp(segs: &[Segment], indices: &[usize]) -> Option<Vector3<f32>> {
...
fn build_lsd_diagnostics(
    ...
    LsdDiagnostics {
        ...
        dominant_angles_deg: [angle1.to_degrees(), angle2.to_degrees()],
