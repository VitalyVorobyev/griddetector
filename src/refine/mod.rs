...
pub mod homography {
    ...
    //!
    //! The refiner consumes bundles produced by the detector, assigns them to
    //! one of two dominant vanishing-point families implied by the current
    //! homography, and performs a Huber-weighted IRLS update of the vanishing
    //! points and anchor column.
    ...
    /// 5. Stop early once the Frobenius improvement drops below a small
    ///    threshold or no valid update is found.
    pub struct Refiner {
        params: RefineParams,
    }
    ...
    impl Refiner {
        ...
        pub fn refine(
            ...
            for level in levels.iter().rev() {
                ...
                let buckets =
                    match families::split_bundles(&current_h, level.bundles, orientation_tol) {
                        Some(b) => b,
                        None => {
                            level_reports.push(level_diag);
                            continue;
